...
import net from 'net';
import start from '../vscode/vs/workbench/api/node/extensionHostProcess.js';
import { FileRPCProtocolLogger } from '../vscode/vs/workbench/services/extensions/common/fileRPCProtocolLogger.js';
import { RequestInitiator } from '../vscode/vs/workbench/services/extensions/common/rpcProtocol.js';
...
const PORT = parseInt(args.find(arg => arg.startsWith('--vscode-socket-port='))?.substring(21) || 
                     process.env.VSCODE_EXTHOST_DEBUG_PORT || '51234', 10);
const SOCKET_HOST = args.find(arg => arg.startsWith('--vscode-socket-host='))?.substring(21) || process.env.VSCODE_EXTHOST_SOCKET_HOST || '127.0.0.1';
const WILL_SEND_SOCKET = args.find(arg => arg.startsWith('--vscode-will-send-socket='))?.substring(26) || process.env.VSCODE_EXTHOST_WILL_SEND_SOCKET || '0';
...
function startServer() {
    const server = net.createServer((socket) => {
        ...
        socket.on('close', () => {
            ...
            fileLoggerGlobal.logOutgoing(0, 0, RequestInitiator.LocalSide, 'Client connection closed, waiting for new connections...');
        });
    });
